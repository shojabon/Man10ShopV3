...
class MoneyRefillFunction(ShopFunction):
    allowed_shop_type = []
    ...
    def calculate_money_to_refill(self):
        if datetime.datetime.now().timestamp() - self.get_last_refill_time() >= self.get_time() * 60:
            ...
            self.shop.money_function.set_money(self.get_amount())
    ...
    def per_minute_execution_task(self):
        self.calculate_money_to_refill()